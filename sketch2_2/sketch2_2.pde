...
class Disc{
  ...
  ArrayList<Disc> plist = new ArrayList<Disc>(100);
...
if(it1>it){
  ...
  // program this
    Disc pDisc = new Disc(x,y,10);
    if(isFarEnough(x,y,plist)){
    plist.add(pDisc);
...
{
  int px = (int)random(5,w-5);
  int py = (int)random(5,w-5);
  
  if(isFarEnough(px,py,plist)){
  float it = getAvgIntensity(px-5,py-5,px+5,py+5,S);
  float radius=1/(0.1+1-it);
  
  plist.add(new Disc(px,py,radius));
