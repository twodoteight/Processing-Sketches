...
void movePoints() {
  ...
  for (int x = 0; x < width; x++ ) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      weight = 1.0-(brightness(inp.get(x, y))/255);
      pointList.get(ic).tx += x*weight; // sum up pixel positions of that point
      pointList.get(ic).ty += y*weight;
      pointList.get(ic).n += weight; // sum pixels that belong to that point
    }
  }
...
void setup() {
  ...
  inp = loadImage("data/input1.jpg");
  inp.resize(0, 500); // proportional scale to height=1000
