...
PImage silhouette(PImage img, PImage depth, PImage nmap, float epsilon) {
  ...
  for (int y=0; y<h; y++) { //<>//
    for (int x=0; x<w; x++) {
      res.pixels[x+y*w] = white;
      float nv = viewVector.dot(RGBtoNormal(x,y,nmap));
      if (abs(nv)<epsilon) {
      res.pixels[x+y*w] = black;
      }
    } //<>//
  }
  return res;
...
PVector RGBtoNormal(float x, float y, PImage nmap) {
  color rgb = nmap.get(int(x),int(y));
  float r = red(rgb);
  float g = green(rgb);
  float b = blue(rgb);

  PVector rev = new PVector(r/255, g/255, -b/255);
  return rev;
}
...
void keyPressed() {
  ...
  float epsilon = mousePos/inp.width;
  println(epsilon);
  outp = silhouette(inp, depth, nmap,epsilon);  
  }
}
